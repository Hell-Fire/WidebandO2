...
PID heater(&heatIn, &heatOut, &heatTarget, 600, 200, 50, REVERSE);
double pumpIn, pumpOut;
PID pump(&pumpIn, &pumpOut, &pumpTarget, 100, 1, 1, DIRECT);

const int running = 10;

RunningAverage vgndavg(running);
RunningAverage ipaavg(running);
RunningAverage nernstavg(running);
